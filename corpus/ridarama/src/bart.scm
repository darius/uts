...
(define make-trip-request
  (lambda (start dest time-constraint)
    (variant-case time-constraint
      (leaving-at (time)
	(build-trip-computation time start dest #t))
      (arriving-at (time)
	(build-trip-computation time dest start #f)))))

(define build-trip-computation
  (lambda (time search-origin search-goal forwards?)
    ...
    (define hastier?
      (lambda (base-time arc1 arc2)

	(define interval
	  (lambda (arc)
	    (let ((e (arc.endpoints arc)))
	      ; This is a hack -- if the arc straddles BASE-TIME we should
	      ; be adding 86400 to the smaller interval.  (Right?)
	      ; And elsewise we should be ignoring the `origin' endpoint.
	      (max (time-interval base-time (node.time (vector-ref e 0)))
		   (time-interval base-time (node.time (vector-ref e 1)))))))

	(< (interval arc1) (interval arc2))))

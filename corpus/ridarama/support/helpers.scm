...
;; Return the element of LS whose KEY is smallest.
;; Pre: LS is nonempty.
(define min-element
  (lambda (key ls)
    (let checking ((smallest (car ls)) (ls (cdr ls)))
      (if (null? ls)
	  smallest
	  (checking (if (< (key (car ls)) (key smallest))
			(car ls)
			smallest)
		    (cdr ls))))))
...
; Return a list of all nonempty partitions of LS by KEY.
;
; That is, if result = L, then (foldr append '() L) contains each
; element of LS exactly once (and no other elements), and (all
; same-keys? L) is true, where (same-keys? PART) is true iff (key x)
; = (key y) for every x and y in PART.  Also no partition is null.
; Finally, the elements of each partition are in the same order as
; they are in LS.
(define partition
  (lambda (key ls)
    ...
    (define add
      (lambda (x parts)
	(let ((k (key x)))
	  (let checking ((parts parts))
	    (cond ((null? parts) 
		   (list (list x)))
		  ((equal? k (key (caar parts)))
		   (cons (cons x (car parts))
			 (cdr parts)))
		  (else
		   (cons (car parts)
			 (checking (cdr parts)))))))))
...
;; Return the smallest i for which (FOUND? (vector-ref VEC i)) is true,
;; or #f if none.
(define vector-find-index
  (lambda (found? vec)
    (let ((limit (vector-length vec)))
      (let looking ((i 0))
	(cond ((= i limit) 
	       #f)
	      ((found? (vector-ref vec i)) 
	       i)
	      (else
	       (looking (+ i 1))))))))

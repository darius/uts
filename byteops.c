...
case bop_varref: { 
  ...
  if (frame == 0) {                         // break out stats for the innermost frame
...
case bop_prim_0: {
  switch (get_byte()) {
    ...
    break;
    case p0_current_input_port: {
      acc = current_input_port;
    }

    break;
    case p0_current_output_port: {
      acc = current_output_port;
    }

    break;
    case p0_runtime: {
      acc = make_flonum((clock() - clock_start) / (double)CLOCKS_PER_SEC);
    }
  }
  push(acc);
}
...
#define callout1(c)   do { c_routine = (c); goto callout1_label; } while (0)
  ...
  switch (get_byte()) {
    ...
    case p1_integerP: {
      acc = make_boolean(is_fixnum(x0)
                         || (is_flonum(x0)
                             && flonum_value(x0) == floor(flonum_value(x0))));
    }
    ...
    break;
    case p1_integerTOchar: {
      callout1(integer_to_char);
    }

    break;
    case p1_charTOinteger: {
    ...
    break;
    case p1_closureTOrenv: {
      vm_check_type(is_closure(x0), x0);
      acc = closure_renv(x0);
    }

    break;
    case p1_closureTOcode: {
      vm_check_type(is_closure(x0), x0);
      acc = closure_code(x0);
    }
...
#define callout2(c)   do { c_routine = (c); goto callout2_label; } while (0)
  ...
  switch (get_byte()) {
    ...
    case p2_write_char: {
      ...
      // FIXME: should fflush() here, but it'd really kill performance...
    }
...
#define must_be_natnum(obj)                    \
    ...
    break;
    case p2_numberTOstring: {
      callout2(prim_number_to_string);
    }

    break;
    case p2_stringTOnumber: {
    ...
    case p2_arithmetic_shift: {
      ...
      if (n >= 0) {
        Fixnum value = v << n;
        if (!int_is_fixnum(value) || (value >> n) != v) {
          error_msg = "Overflow in arithmetic shift";
          acc = cons(x0, cons(x1, nil));
          goto vm_error_label;
        }
        acc = make_fixnum(value);
      } else {
